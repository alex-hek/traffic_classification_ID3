...
class Flow(object):
    ...
    def __init__(self, src_ip, dst_ip, src_mac, dst_mac, src_port, dst_port, trans_layer_proto, packet, timestamp,
        ...
        self.length = length
        self.pktsizeseq = list()
        self.pktsizeseq.append(pkt_size)

    def append_packet(self, packet, timestamp, pktsize):
        self.packets.append(packet)
        self.timestamps.append(timestamp)
        self.pktsizeseq.append(pktsize)
...
def extractFlow(flow_list):
    ...
    for idx in range(len(flow_list)):
        ...
        '''
        flow = flow_list[idx]
        five_tuple = [flow.src_ip, flow.dst_ip, flow.src_port, flow.dst_port, flow.trans_layer_proto]
        pkt_len_seq = flow.pktsizeseq
        session_list.append([five_tuple, pkt_len_seq])
    return session_list
